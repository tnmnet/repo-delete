# USCIS SPEED Challenge Platform
This application will create the USCIS SPEED Challenge platform including the Kubernetes cluster and additional tools.
The high-level architecture overview shows the platform at a glance.

![USCIS SPEED Challenge high-Level architecture](docs/DemoHLA.png "USCIS SPEED Challenge high-Level architecture")
...
- **config.yaml**: contains the workspaces currently configured for this project, an example config.sample.yaml file can be found in the `config` directory of the Git project
- **secrets.yaml**: contains the secrets needed for the application, you'll find examples in the `secrets.sample.yaml` file, workspace names between **config.yaml** and **secrets.yaml** need to correlate
...
# WKS (workspace): a workspace configured in your config.yaml
docker run -it \
